...
object ActorSystemImpl {
  ...
  case class CreateSystemActor[T](behavior: Behavior[T], deployment: DeploymentConfig)(val replyTo: ActorRef[ActorRef[T]]) extends SystemCommand

  val systemGuardianBehavior: Behavior[SystemCommand] =
    ContextAware { ctx ⇒
      Static {
        case create: CreateSystemActor[t] ⇒
          create.replyTo ! ctx.spawnAnonymous(create.behavior, create.deployment)
      }
    }
}
...
private[typed] class ActorSystemImpl[-T](
  ...
  def systemActorOf[U](behavior: Behavior[U], name: String, deployment: DeploymentConfig)(implicit timeout: Timeout): Future[ActorRef[U]] = {
    ...
    systemGuardian ? CreateSystemActor(behavior, deployment)
  }
